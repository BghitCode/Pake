...
通过样式修改、JavaScript 注入和容器通信等方式自定义 BghitApp 应用。
...
要求：BghitApp CLI >= 3.0.0

## macOS 摄像头与麦克风权限

BghitApp 构建的应用默认不申请摄像头或麦克风权限。对于需要这些权限的站点（例如视频会议或语音输入），在构建时传入对应的标志：
...
BghitApp 现在会基于 `URL + name` 生成不同的应用标识，因此这两个应用会被当作两个独立桌面应用安装，而不是落到同一个应用上。

对于需要固定 bundle identifier 的高级场景，BghitApp 也支持一个隐藏参数 `--identifier`：
...
了解 BghitApp 的代码库结构将帮助您有效地进行导航和贡献：
...
此脚本会读取上述配置并使用 watch 模式打包指定的应用，对 `@bghitcode/bghitapp` 代码修改可实时热更新。
...
- `--e2e`：增加端到端配置测试
- `--@bghitcode/bghitapp`：增加 GitHub Actions 相关检查
...
- [GitHub 讨论区](https://github.com/BghitCode/bghitapp/discussions)
